...
var: ContextVar = ContextVar('var')

codes = [
    "56FF4D", "A3D2F7", "B1C94A", "F56A1D", "D4E6F1",
...
def get_code(task: asyncio.Task) -> None:
    code: str = var.get()
    print(
        f'{task.result()}' if int(code, 16) % 2 else
        f'Сообщение: Неверный код, сообщение скрыто',
        f'Код: {code}',
        sep='\n'
    )
