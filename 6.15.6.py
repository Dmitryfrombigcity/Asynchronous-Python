...
async def check_message(message: dict[str, int | str]) -> None:
    message_mod = map(lambda x: x.strip('.,?'), message.get('message').lower().split())
    await asyncio.sleep(0)
    if len(set(message_mod).intersection(banned_words)):
        print(f'В сообщении {message.get("message_id")} стоп-слово: task.done(): False')
    else:
...
async def main() -> None:
    await asyncio.gather(
        *(check_message(message) for message in messages),
