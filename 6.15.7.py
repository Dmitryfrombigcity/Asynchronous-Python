...
async def check_message(message: dict[str, Any]) -> None:
    banned_words_in_message = [
        word for word in banned_words if word in message["message"].lower()
    ]
    await asyncio.sleep(0)

    match asyncio.current_task().get_name():  # type: ignore
        case 'admin':
            message_mod = f'admin: {message["message"]}'
        case 'moderator':
            if len(banned_words_in_message):
                message_with_banned_words = message["message"]
                for banned_word in banned_words_in_message:
                    message_with_banned_words = message_with_banned_words.replace(banned_word, "****")
                message_mod = f'moderator: {message_with_banned_words}'
            else:
                message_mod = f'moderator: {message["message"]}'
        case 'student':
            message_mod = 'student: В сообщении есть запрещённое слово, сообщение скрыто' \
                if len(banned_words_in_message) else \
                f'student: {message["message"]}'
        case 'black_list_user':
            message_mod = 'black_list_user: Пользователь забанен, сообщение скрыто'
        case _:
            message_mod = 'None: ERROR_USER_NONE'
    print(message_mod)


async def main() -> None:
    tasks = (
        asyncio.create_task(check_message(message), name=message['role']) for message in messages
    )
